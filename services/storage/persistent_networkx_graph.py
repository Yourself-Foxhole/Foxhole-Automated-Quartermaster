...
class PersistentNetworkXGraph(Persistent):
    ...
    def __init__(self, graph_type: str = "DiGraph", data=None, **kwargs):
        ...
        logger.debug(f"Created persistent {graph_type} with {len(self._graph.nodes)} nodes and {len(self._graph.edges)} edges")
            ...
            if callable(attr):
                def wrapper(*args, **kwargs):
                    ...
                    if not self._batch_mode and name in [
                        'clear', 'update', 'add_weighted_edges_from',
                        'remove_nodes_from', 'remove_edges_from'
                    ]:
                        self._sync_to_persistent()
                    return result
                return wrapper
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'is_connected': nx.is_connected(self._graph) if not self.is_directed() else nx.is_weakly_connected(self._graph),
    ...
    def validate_integrity(self) -> Dict[str, Any]:
        ...
        if nx_nodes != persistent_nodes:
            issues.append(f"Node mismatch: NetworkX has {len(nx_nodes)}, persistent has {len(persistent_nodes)}")
        ...
        if nx_edges != persistent_edges:
            issues.append(f"Edge mismatch: NetworkX has {len(nx_edges)}, persistent has {len(persistent_edges)}")
