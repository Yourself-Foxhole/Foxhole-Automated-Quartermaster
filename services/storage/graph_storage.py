...
class GraphStorage(Persistent):
        ...
        self.metadata.update(
            {
                ...
                "last_modified": None,
                "total_graphs": 0,
                "graph_types": PersistentList(
                    ["production", "task", "inventory", "transport"]
                ),
    ...
    def register_graph(
        ...
        if graph_type not in self.metadata["graph_types"]:
            raise ValueError(
                f"Unknown graph type: {graph_type}. Supported types: {list(self.metadata['graph_types'])}"
            )
    ...
    def validate_integrity(self) -> Dict[str, Any]:
        ...
        for graph_id, info in self.graph_registry.items():
            ...
            if graph_id not in storage_container:
                issues.append(
                    f"Graph {graph_id} registered but not found in {graph_type} storage"
                )
        ...
        for graph_type in self.metadata["graph_types"]:
            ...
            for graph_id in storage_container.keys():
                if graph_id not in self.graph_registry:
                    issues.append(
                        f"Graph {graph_id} found in {graph_type} storage but not registered"
                    )
