...
class PersistentProductionNode(PersistentBaseNode):
    ...
    def __init__(self, node_id: Optional[str] = None, name: str = "", facility_type: str = "factory", **kwargs):
        """
        ...
        logger.debug(f"Created production node: {self.node_id} ({facility_type}) at {self.location}")
    ...
    def start_production(self, recipe_id: str, cycles: int = 1, priority: str = "normal") -> bool:
        """
        ...
        required_tier = recipe.get('required_tier', 1)
        if required_tier not in self.unlocked_tiers and required_tier > self.tech_tier:
            logger.warning(f"Tech tier {required_tier} not unlocked for recipe {recipe_id}")
            return False
        ...
        for input_item, required_qty in inputs.items():
            ...
            if available_qty < total_needed:
                logger.warning(f"Insufficient {input_item} for production: {available_qty} < {total_needed}")
                return False
        ...
        logger.info(f"Started production of {recipe_id} for {cycles} cycles at {self.name}")
        return True
    ...
    def cancel_production(self, recipe_id: str) -> bool:
        ...
        if recipe:
            remaining_cycles = production_order['cycles'] - production_order['cycles_completed']
            inputs = recipe.get('inputs', {})
    ...
    def remove_output_material(self, material_name: str, quantity: float) -> bool:
        ...
        if current_qty < quantity:
            logger.warning(f"Insufficient {material_name} in output storage: {current_qty} < {quantity}")
            return False
