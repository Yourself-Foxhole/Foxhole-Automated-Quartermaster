...
class PersistentTaskNode(PersistentBaseNode):
    ...
    def __init__(self, node_id: Optional[str] = None, name: str = "", task_type: str = "logistics", **kwargs):
        """
        ...
        self.task_status = kwargs.get('task_status', 'pending')  # pending, active, completed, failed, cancelled
        ...
        self.priority_level = kwargs.get('priority_level', 'normal')  # low, normal, high, critical
        self.priority_score = kwargs.get('priority_score', 50.0)  # 0-100 numeric priority
        self.resource_requirements = PersistentMapping(kwargs.get('resource_requirements', {}))
    ...
    def add_dependency(self, task_id: str, dependency_type: str = "blocks") -> bool:
        ...
        for existing in self.dependencies:
            if existing['task_id'] == task_id:
                logger.warning(f"Dependency {task_id} already exists for task {self.node_id}")
                return False
    ...
    def start_task(self, assigned_to: Optional[str] = None) -> bool:
        ...
        if self.task_status != 'pending':
            logger.warning(f"Cannot start task {self.node_id} with status {self.task_status}")
            return False
    ...
    def update_progress(self, percentage: float, message: str = "") -> bool:
        ...
        if self.task_status not in ['active', 'pending']:
            logger.warning(f"Cannot update progress for task {self.node_id} with status {self.task_status}")
            return False
    ...
    def complete_task(self, success: bool = True, results: Optional[Dict[str, Any]] = None) -> bool:
        """
        Complete the task.
        ...
        """
        if self.task_status not in ['active', 'pending']:
            logger.warning(f"Cannot complete task {self.node_id} with status {self.task_status}")
            return False
        ...
        if self.actual_start:
            self.actual_duration = (self.actual_completion - self.actual_start).total_seconds()
    ...
    def cancel_task(self, reason: str = "") -> bool:
        ...
        if self.task_status in ['completed', 'cancelled']:
            logger.warning(f"Cannot cancel task {self.node_id} with status {self.task_status}")
            return False
    ...
    def retry_task(self) -> bool:
        ...
        if self.task_status != 'failed':
            logger.warning(f"Cannot retry task {self.node_id} with status {self.task_status}")
            return False
        
        if self.retry_count >= self.max_retries:
            logger.warning(f"Max retries ({self.max_retries}) exceeded for task {self.node_id}")
            return False
        ...
        logger.info(f"Retrying task {self.node_id}: {self.name} (attempt {self.retry_count})")
        return True
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        task_dict = {
            ...
            'allocated_resources': {k: dict(v) for k, v in self.allocated_resources.items()},
