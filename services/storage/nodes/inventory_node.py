...
class PersistentInventoryNode(PersistentBaseNode):
    ...
    def __init__(self, node_id: Optional[str] = None, name: str = "", location: str = "", **kwargs):
        """
    ...
    def add_inventory(self, item_name: str, quantity: float, quality: str = "standard") -> bool:
        """
        ...
        if not self._can_accommodate(item_name, quantity):
            logger.warning(f"Cannot accommodate {quantity} of {item_name} at {self.name}")
            return False
    ...
    def remove_inventory(self, item_name: str, quantity: float, quality: str = "standard") -> bool:
        """
        ...
        if current < quantity:
            logger.warning(f"Insufficient {item_name} ({quality}) at {self.name}: {current} < {quantity}")
            return False
    ...
    def reserve_inventory(self, item_name: str, quantity: float, reservation_id: str, quality: str = "standard") -> bool:
        """
        ...
        if available < quantity:
            logger.warning(f"Insufficient available {item_name} ({quality}) for reservation: {available} < {quantity}")
            return False
        ...
        logger.debug(f"Reserved {quantity} {item_name} ({quality}) with ID {reservation_id}")
        return True
    
    def release_reservation(self, item_name: str, reservation_id: str, quality: str = "standard") -> bool:
        """
        ...
        self.update_modified_time()
        logger.debug(f"Released reservation {reservation_id} for {item_name} ({quality})")
        return True
    
    def get_current_inventory(self, item_name: Optional[str] = None, quality: Optional[str] = None) -> Dict[str, float]:
        """
        ...
        if item_name and quality:
            ...
            return {k: v for k, v in self.current_inventory.items() if k.startswith(f"{item_name}:")}
        else:
            return dict(self.current_inventory)
    
    def get_available_inventory(self, item_name: str, quality: str = "standard") -> float:
        """
    ...
    def get_reserved_inventory(self, item_name: Optional[str] = None, quality: Optional[str] = None) -> Dict[str, Dict[str, float]]:
        """
        ...
        if item_name and quality:
            ...
            return {k: dict(v) for k, v in self.reserved_inventory.items() if k.startswith(f"{item_name}:")}
        else:
    ...
    def _can_accommodate(self, item_name: str, quantity: float) -> bool:
        ...
        if item_name in self.capacity_by_type:
            current_item = sum(v for k, v in self.current_inventory.items() if k.startswith(f"{item_name}:"))
    ...
    def get_capacity_utilization(self) -> Dict[str, float]:
        ...
        utilization = {
            ...
            'utilization_percent': (current_total / self.max_capacity * 100) if self.max_capacity > 0 else 0,
        ...
        for item_name, capacity in self.capacity_by_type.items():
            current_item = sum(v for k, v in self.current_inventory.items() if k.startswith(f"{item_name}:"))
            utilization[f"{item_name}_utilization"] = (current_item / capacity * 100) if capacity > 0 else 0
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        inventory_dict = {
            ...
            'reserved_inventory': {k: dict(v) for k, v in self.reserved_inventory.items()},
