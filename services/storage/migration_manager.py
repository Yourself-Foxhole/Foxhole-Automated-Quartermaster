...
from services.production_calculator.production_graph import ProductionGraph, ProductionNode
...
class GraphMigrationManager:
    ...
    def migrate_production_graph_from_pickle(self, pickle_path: str, graph_id: str) -> bool:
        """
        ...
        try:
            ...
            self.log_migration(f"Loaded ProductionGraph with {len(production_graph.all_nodes())} nodes")
            
            # Create backup
            self._create_backup(pickle_path)
            
            # Convert to persistent graph
            persistent_graph = self._convert_production_graph(production_graph, graph_id)
            
            # Validate migration
            if self._validate_production_graph_migration(production_graph, persistent_graph):
                self.log_migration(f"Successfully migrated ProductionGraph to {graph_id}")
                return True
            else:
        ...
        try:
            ...
            for entry in cache_entries:
                try:
                    ...
                    self.log_migration(f"Failed to migrate cache entry for {entry.node_name}: {e}", "warning")
    ...
    def _convert_production_graph(self, production_graph: ProductionGraph, graph_id: str) -> PersistentNetworkXGraph:
        """
            ...
            for node_name in production_graph.all_nodes():
                ...
                for recipe in production_node.recipes:
                    recipe_data = {
                        ...
                        'outputs': recipe.outputs or {production_node.name: getattr(recipe, 'output_per_cycle', 1)},
                ...
                # Set node category
                persistent_node.set_attribute('original_category', production_node.category)
        ...
        self.log_migration(f"Converted {len(production_graph.all_nodes())} nodes and {len(production_graph.graph.edges)} edges")
        
        return persistent_graph
    
    def _validate_production_graph_migration(self, original: ProductionGraph, migrated: PersistentNetworkXGraph) -> bool:
        """
        ...
        if len(original_nodes) != len(migrated_nodes):
            validation_issues.append(f"Node count mismatch: {len(original_nodes)} -> {len(migrated_nodes)}")
        ...
        if original_edges != migrated_edges:
            validation_issues.append(f"Edge count mismatch: {original_edges} -> {migrated_edges}")
        ...
        for node_name in original_nodes & migrated_nodes:
            ...
            if 'persistent_node' in migrated_node_data:
                ...
                if original_category != original_node.category:
                    validation_issues.append(f"Category mismatch for {node_name}: {original_node.category} -> {original_category}")
    ...
    def export_migration_report(self, output_path: Optional[str] = None) -> str:
        ...
        if output_path is None:
            output_path = os.path.join(self.backup_dir, f"migration_report_{self.current_migration_id}.json")
        ...
        report = {
            ...
            'summary': {
                ...
                'warnings': len([e for e in self.migration_log if e['level'] == 'warning']),
    ...
    def rollback_migration(self, migration_id: str) -> bool:
        ...
        try:
            ...
            self.log_migration("Rollback completed (implementation simplified)", "warning")
            return True
    ...
    def get_migration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'errors_count': len([e for e in self.migration_log if e['level'] == 'error']),
            'warnings_count': len([e for e in self.migration_log if e['level'] == 'warning'])
        }
